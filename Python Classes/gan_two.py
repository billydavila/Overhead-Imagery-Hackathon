...
class GanTwo:
    ...
    def generator(self):
        ...
        last = Conv2DTranspose(filters=self.__OUTPUT_CHANNELS, kernel_size=4,
                               strides=2, padding="same", kernel_initializer=init, activation="tanh")
        x = inputs
    ...
    def discriminator(self):
        ...
        tar = Input(shape=[self.__IMAGE_SIZE,
                           self.__IMAGE_SIZE, 4], name="target_image")
    ...
    def save_images(self, model, test_input, target, epoch):
        ...
        mask_pred, post_image_pred = tf.experimental.numpy.split(prediction, [
                                                                 1], axis=-1)
        mask_pred = tf.squeeze(mask_pred)

        display_list = [test_input[0], target[0],
                        prediction[0], mask_pred, post_image_pred[0]]

        title = ["Input Image (Pre-disaster + Post-Mask)", "Ground Truth(Post Disaster + Post-Mask)",
                 "Prediction Image (Post-Disaster)", "Mask Prediction", "Post Prediction"]
        ...
        if epoch % 25 == 0:
            plt.savefig(
                f"drive/My Drive/Colab Notebooks/modified_data/pre_and_post_joined/output_GAN2/epoch_{epoch}.png")
    ...
    def predict_from_path(self, pre_image_path, mask_image_path):
        ...
        predictions = self.gen(stacked_image)

        mask_pred, post_image_pred = tf.experimental.numpy.split(predictions, [
                                                                 1], axis=-1)
        mask_pred = tf.squeeze(mask_pred)
        post_image_pred = tf.squeeze(mask_pred)

        return mask_pred, post_image_pred
